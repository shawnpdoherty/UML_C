...
int main ( )
{
    ...
    for (int i = 0; i < SIZE; ++i)
    {
        ...
        // update totals
        total[0] += wage[i]; // sub 0 for wage total
        total[1] += hours_worked[i]; // sub 1 for hours total 
        total[2] += overtime_hours[i]; // sub 2 for ot total       
        total[3] += gross[i]; // sub 3 for gross total
    }
    ...
    // iterate through array to print to file 
    for (int i = 0; i < SIZE; ++i)
    {
